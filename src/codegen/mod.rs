...
use crate::memory::lifetime::{is_refcounted, LifetimeTracker};
...
impl CodeGenerator {
    ...
    fn emit_runtime_declarations(&mut self) {
        self.builder
            .emit_module("; LYZARD runtime function declarations");
        self.builder.emit_module("declare void @lyz_print_int(i64)");
        self.builder
            .emit_module("declare void @lyz_print_float(double)");
        ...
        self.builder
            .emit_module("declare ptr @lyz_slice(ptr, i64, i64)");
    ...
    fn compile_fn(&mut self, decl: &FnDecl) -> Result<(), RuntimeError> {
        ...
        let param_decls: Vec<String> = decl
            .params
            .iter()
            .filter(|p| !p.is_self)
        ...
        for param in decl.params.iter().filter(|p| !p.is_self) {
            ...
            self.builder
                .emit_store("i64", &format!("%{}", param.name), &ptr);
            self.locals.insert(
                param.name.clone(),
                VarLocation {
                    ptr,
        ...
        let param_types: Vec<(String, ResolvedType)> = decl
            .params
            .iter()
            .filter(|p| !p.is_self)
    ...
    fn compile_fn_body_with_rc(
        &mut self,
        block: &Block,
        param_types: &[(String, ResolvedType)],
    ) -> Result<(), RuntimeError> {
    ...
    fn emit_retain(&mut self, ptr_val: &str) {
        self.builder
            .emit(format!("call void @lyz_retain(ptr {})", ptr_val));
    }

    /// Emit `call void @lyz_release(ptr %val)`
    fn emit_release(&mut self, ptr_val: &str) {
        self.builder
            .emit(format!("call void @lyz_release(ptr {})", ptr_val));
    }
    ...
    fn compile_call_arg_with_retain(
        &mut self,
        expr: &Expr,
    ) -> Result<(String, ResolvedType), RuntimeError> {
        let (val, ty) = self.compile_expr(expr)?;
        if is_refcounted(&ty) {
            self.emit_retain(&val);
        }
    ...
    fn try_compile_intrinsic_call(
        ...
        for (name, runtime_fn, ret_ty) in INTRINSICS {
            if *name == fn_name {
                let result = self
                    .builder
                    .emit_call(&llvm_type(ret_ty), runtime_fn, arg_pairs);
    ...
    fn compile_if(&mut self, stmt: &IfStmt) -> Result<(), RuntimeError> {
        ...
        self.builder
            .emit_cond_branch(&cond_val, &then_label, false_target);
    ...
    fn compile_while(&mut self, stmt: &WhileStmt) -> Result<(), RuntimeError> {
        ...
        let (cond_val, _) = self.compile_expr(&stmt.condition)?;
        self.builder
            .emit_cond_branch(&cond_val, &body_label, &exit_label);
    ...
    fn compile_expr(&mut self, expr: &Expr) -> Result<(String, ResolvedType), RuntimeError> {
        match expr {
            ...
            Expr::StructInit(s) => {
                ...
                let raw = self.builder.emit_call(
                    "ptr",
                    "lyz_alloc",
                    &[
                ...
                let fn_name = match c.callee.as_ref() {
                    ...
                    _ => {
                        return Err(RuntimeError::NotImplemented {
                            feature: "indirect calls".to_string(),
                        })
                    }
                };
            ...
            Expr::Assign(a) => {
                ...
                let id = match a.target.as_ref() {
                    ...
                    _ => {
                        return Err(RuntimeError::NotImplemented {
                            feature: "assignment to non-local target".to_string(),
                        })
                    }
                };
                let loc =
            ...
            _ => Err(RuntimeError::NotImplemented {
                feature: "this expression in codegen".to_string(),
            }),
        }
    }
    ...
    fn compile_binary(
        ...
        let result = match op {
            BinaryOp::Add => {
                self.builder
                    .emit_binop(if is_float { "fadd" } else { "add" }, &llty, &left, &right)
            }
            BinaryOp::Sub => {
                self.builder
                    .emit_binop(if is_float { "fsub" } else { "sub" }, &llty, &left, &right)
            }
            BinaryOp::Mul => {
                self.builder
                    .emit_binop(if is_float { "fmul" } else { "mul" }, &llty, &left, &right)
            }
            BinaryOp::Div => self.builder.emit_binop(
                if is_float { "fdiv" } else { "sdiv" },
                &llty,
                &left,
                &right,
            ),
            BinaryOp::Mod => self.builder.emit_binop(
            ...
            BinaryOp::Eq => {
                return Ok((
                    self.builder.emit_icmp("eq", &llty, &left, &right),
                    ResolvedType::Bool,
                ))
            }
            BinaryOp::NotEq => {
                return Ok((
                    self.builder.emit_icmp("ne", &llty, &left, &right),
                    ResolvedType::Bool,
                ))
            }
            BinaryOp::Lt => {
                return Ok((
                    self.builder.emit_icmp("slt", &llty, &left, &right),
                    ResolvedType::Bool,
                ))
            }
            BinaryOp::Lte => {
                return Ok((
                    self.builder.emit_icmp("sle", &llty, &left, &right),
                    ResolvedType::Bool,
                ))
            }
            BinaryOp::Gt => {
                return Ok((
                    self.builder.emit_icmp("sgt", &llty, &left, &right),
                    ResolvedType::Bool,
                ))
            }
            BinaryOp::Gte => {
                return Ok((
                    self.builder.emit_icmp("sge", &llty, &left, &right),
                    ResolvedType::Bool,
                ))
            }
            ...
            BinaryOp::And => {
                return Ok((
                    self.builder.emit_binop("and", "i1", &left, &right),
                    ResolvedType::Bool,
                ))
            }
            BinaryOp::Or => {
                return Ok((
                    self.builder.emit_binop("or", "i1", &left, &right),
                    ResolvedType::Bool,
                ))
            }
        };
...
"#,
    ...
    fn test_multiple_locals_released_in_reverse_order() {
        ...
        // line is not a call)
        let release_calls: Vec<usize> = ir
            .match_indices("call void @lyz_release")
            .map(|(i, _)| i)
            .collect();
        assert_eq!(
            release_calls.len(),
            2,
            "Both string locals should be released"
        );
    }
...
mod codegen_expr_tests {
    ...
    fn gen() -> CodeGenerator {
        ...
        g.builder.start_function();
        g
    }

    fn dummy_span() -> crate::lexer::Span {
        crate::lexer::Span::dummy()
    }
    ...
    fn test_add_emits_add_instruction() {
        let mut g = gen();
        let l = Expr::Int(IntLit {
            value: 3,
            span: dummy_span(),
        });
        let r = Expr::Int(IntLit {
            value: 4,
            span: dummy_span(),
        });
