...
fn run_with_stdlib(user_code: &str) -> Result<(), String> {
    ...
    let (prog, errs) = Parser::new(tokens, "test.lyz", &full_src).parse().map_err(|e| e.to_string())?;
    if !errs.is_empty() { return Err(errs.format_all(&full_src)); }
